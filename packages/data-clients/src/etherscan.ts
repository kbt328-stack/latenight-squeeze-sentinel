...
function baseUrl(chain: Chain): string {
  const chainId: Partial<Record<Chain, string>> = {
    ethereum: '1',
    base: '8453',
    arbitrum: '42161',
  };
  const id = chainId[chain];
  if (!id) throw new SentinelError(ErrorCodes.UPSTREAM_ERROR, `Unsupported chain: ${chain}`);
  return `https://api.etherscan.io/v2/api?chainid=${id}`;
}
...
export class EtherscanClient {
  ...
  async fetchTotalSupply(contractAddress:string, chain:Chain='ethereum') {
    const url=`${baseUrl(chain)}&module=stats&action=tokensupply&contractaddress=${contractAddress}&apikey=${apiKey(chain)}`;
    return instrumentedFetch<{status:string;message:string;result:string}>(url,{source:SOURCE,endpoint:'tokensupply',rateLimit:RL,apiKey:null,costUnits:1});
  }
