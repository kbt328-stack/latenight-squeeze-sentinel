...
export class CoinglassClient {
  ...
  normalizeFundingRate(raw:CoinglassFundingRateRaw):FundingRateNormalized[] { return (raw.data ?? []).map(item=>{const rates=item.uMarginList.map(e=>e.rate);const avg=rates.length>0?rates.reduce((a,b)=>a+b,0)/rates.length:0;return{symbol:item.symbol,avgAnnualizedRate:avg*3*365*100,byExchange:item.uMarginList.map(e=>({exchange:e.exchangeName,rate:e.rate}))}}); }
  async fetchOpenInterest(symbol:string) { return instrumentedFetch<CoinglassOpenInterestRaw>(`${BASE}/open_interest?symbol=${symbol}`,{source:SOURCE,endpoint:'/openInterest',rateLimit:RL,apiKey:null,headers:{'coinglassSecret':key()},costUnits:1}); }
