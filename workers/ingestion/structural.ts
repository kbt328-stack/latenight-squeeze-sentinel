...
async function ingestToken(token:WatchlistToken):Promise<void>{
  ...
  try{
    ...
    signals.push({tokenId:token.id,plane:PLANE,signalId:'s3',value:eng.activate_s3(hasPerp?1:0),rawPayload:{hasPerp,oiUsd:oi[0]?.totalUsd??0},source:'coinglass',observedAt:now});
  ...
  try{
    ...
    signals.push({tokenId:token.id,plane:PLANE,signalId:'s4',value:eng.activate_s4(n.deployerAppearsDoxxed?0:1),rawPayload:{deployerAppearsDoxxed:n.deployerAppearsDoxxed,riskFlags:n.riskFlags},source:'goplus',observedAt:now});
