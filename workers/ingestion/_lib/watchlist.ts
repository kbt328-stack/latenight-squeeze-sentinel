...
const BOOTSTRAP:WatchlistToken[] = [
  { id:'', symbol:'RAVE', contractAddress:'0x17205fab260a7a6383a81452ce6315a39370db97', chain:'ethereum', coingeckoId:'ravedao' }
];
export async function getWatchlist():Promise<WatchlistToken[]> {
  try {
    ...
    return rows.map(r=>({id:r.id,symbol:r.symbol,contractAddress:r.contractAddress,chain:r.chain,coingeckoId:(r.metadata as Record<string,string>|null)?.['coingeckoId']}));
  } catch(err) {
