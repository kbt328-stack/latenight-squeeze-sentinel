...
async function ingestToken(token:WatchlistToken):Promise<void>{
  ...
  try{
    ...
    const res=await db.execute(`SELECT COUNT(*) as cnt FROM wallet_token_positions WHERE token_id=$1 AND pct_of_circulating>0.01 AND snapshot_at>NOW()-INTERVAL '90 days'`,[token.id]);
