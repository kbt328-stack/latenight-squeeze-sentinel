...
async function ingestToken(token: WatchlistToken): Promise<void> {
  ...
  try {
    if (token.coingeckoId) {
      const chartRaw = await coingecko.fetchMarketChart(token.coingeckoId, 7);
      const candles = coingecko.normalizeMarketChart(chartRaw);
  ...
  try {
    ...
    const insiders = await db.execute(
      sql`SELECT address FROM wallets WHERE 'insider' = ANY(labels) LIMIT 20`
    ) as Array<{ address: string }>;
    let sends = 0; const cutoff = Math.floor((Date.now() - 21_600_000) / 1000);
    for (const w of insiders.slice(0, 10)) {
      try {
        ...
        sends += txs.filter(tx => tx.from === w.address.toLowerCase() && tx.timestamp.getTime() / 1000 > cutoff && EXCHANGES.includes(tx.to)).length;
  ...
  try {
    ...
    signals.push({ tokenId: token.id, plane: PLANE, signalId: 'd5', value: eng.activate_d5(vel.isVolumeATH ? 100 : vel.volumeChangeVs7dAvg), rawPayload: { isVolumeATH: vel.isVolumeATH }, source: 'lunarcrush', observedAt: now });
  } catch (err) { logger.warn({ err, symbol: token.symbol, signals: ['d5'] }, 'LunarCrush distribution failed'); }
